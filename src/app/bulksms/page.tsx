   ...
   const toggleAnswer = (index: number) => {
    ...
    <section className="pt-20">
        <div className='m-5 md:m-20'>
            <h1 className={`text-2xl font-bold dark:text-[#ffee32]  ${UbuntuFont.className}`}>BULK SMS SERVICES</h1>
            <p className={`pt-5 dark:text-[#fefde7] ${InterFont.className}`}>{bulkSms}</p>
            <h1 className={`text-2xl pt-10 font-bold dark:text-[#ffee32]  ${UbuntuFont.className}`}>SEND SMS TO NO. OF PEOPLE IN BULK</h1>
            <p className={`pt-5 dark:text-[#fefde7] ${InterFont.className}`}>{sendSMS}</p>
            <h1 className={`text-2xl pt-10 font-bold dark:text-[#ffee32]  ${UbuntuFont.className}`}>BEST SUPPORT</h1>
            <p className={`pt-5 dark:text-[#fefde7] ${InterFont.className}`}>{bestSupport}</p>
        </div>
        <div className='m-5 lg:m-20'>
            <h1 className={`text-2xl font-bold py-5 dark:text-white ${UbuntuFont.className}`}>FREQUENTLY ASKED QUESTIONS?</h1>
            ...
            <div className='flex pt-5 relative justify-between cursor-pointer p-3'  key={index} >
                <h1 className={`text-2xl font-bold dark:text-[#b80000]  ${UbuntuFont.className}`}>{item.question}</h1>
                <button id="add cursor-pointer " ><IoAdd key={index} className="cursor-poiner" onClick={() => toggleAnswer(index)} /></button>
            </div>
                <div className="p-3">
                <p className={` ${openIndex === index ? `whitespace-pre-wrap font-xl font-bold bg-[#fefde7] p-3 rounded-lg border-x border-b ${InterFont.className}` : 'hidden'}`}>{item.answer}</p>
                </div>
                </div>
)}
</div>
